...
The results of this analysis illustrate clear differences in performance between the linear and radial SVM models and highlight the importance of modeling non-linear relationships when predicting obesity using NHANES variables.

**Model Comparison**

The linear SVM performed poorly, achieving an accuracy of only 63.8% and a sensitivity of 8.7%. Although its specificity was high (91.2%), the model failed to detect most obese participants. This outcome suggests that obesity cannot be effectively separated from non-obesity using a linear decision boundary. This is consistent with the theoretical material presented in the lecture, where SVMs with linear kernels were shown to perform poorly when the classes are not linearly separable.


In contrast, the radial SVM produced substantially better results. With an accuracy of 77.9% and a sensitivity of 63.5%, the model demonstrated a meaningful ability to identify obese individuals. Specificity remained strong (85%), and Cohen’s Kappa increased from 0 (linear) to 0.49, indicating moderate agreement beyond chance. These results align with the advantage of the RBF kernel described in the lecture slides: non-linear kernels can map the data into a higher-dimensional space where a separating boundary becomes feasible.

The progressive improvement in accuracy as the cost parameter C increased (from 0.25 to 128) also mirrors the behavior shown in the lecture’s tuning examples.
Higher values of C allow the SVM to create a more flexible boundary by reducing regularization, which appears appropriate for this dataset.


**Interpretation in the Context of Obesity Prediction**

Obesity is influenced by a combination of demographic, behavioural, socioeconomic, and clinical factors. These relationships are inherently non-linear. For example:

- Individuals with the same income or education may differ significantly in obesity status.
- Blood pressure varies widely among both obese and non-obese groups.
- Physical activity does not perfectly predict obesity, as highly active individuals may still have high BMI.

Because of this complexity, a linear classifier oversimplifies the boundary and cannot capture the underlying structure. The radial SVM, on the other hand, can model curved boundaries and interactions between predictors, producing a more nuanced representation of the data.

Compared with the earlier linear regression model, which assumed a linear relationship between BMI and the predictors, the SVM results show that obesity is better handled using a non-linear classification approach

Nevertheless, even the radial model achieved only moderate performance. This reflects the broader challenge of predicting obesity using survey variables alone. Obesity is multifactorial, and additional predictors - such as diet, genetics, family history, hormone markers, or detailed activity measurements - may be necessary to achieve higher accuracy.
